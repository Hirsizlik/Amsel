...
public class SetStatisticTest
{
    ...
    private static void AssertSetAndTotal(SetStatistic actual, string expSetCode, int expTotal, int expUnique,
        int expCardCount)
    {
        Assert.That(actual.SetCode, Is.EqualTo(expSetCode), $"SetCode {actual.SetCode} {expSetCode}");
    ...
    public void TestSingleSet()
    {
        var actualStatisticDict = SetStatistic.CreateStatistics([
            new CardStats(new CardInfo(1, "Card 1", "TEST", "DTEST", 1, 4, Rarity.Common, true), 1),
            new CardStats(new CardInfo(2, "Card 2", "TEST", "DTEST", 2, 4, Rarity.Common, true), 2),
            new CardStats(new CardInfo(3, "Card 3", "TEST", "DTEST", 3, 4, Rarity.Rare, true), 3),
            new CardStats(new CardInfo(4, "Card 4", "TEST", "DTEST", 4, 4, Rarity.MythicRare, true), 0),
            new CardStats(new CardInfo(5, "Card Bonus", "TEST", "DTEST", 4, null, Rarity.MythicRare, true), 4),
        ]);
        ...
        using (Assert.EnterMultipleScope())
        {
            foreach (string s in expectedSets)
            {
                ...
                AssertSetAndTotal(actual, s, 10, 4, 5);
    ...
    {
        var actualStatisticDict = SetStatistic.CreateStatistics([
            new CardStats(new CardInfo(1, "Card 1", "T1", "D1", 1, 1, Rarity.Common, true), 1),
            new CardStats(new CardInfo(2, "Card 2", "T2", "D1", 2, 3, Rarity.Common, true), 2),
            new CardStats(new CardInfo(3, "Card 3", "T2", "D2", 3, null, Rarity.Common, true), 3),
        ]);
        ...
        using (Assert.EnterMultipleScope())
        {
            AssertSetAndTotal(actualT1, "T1", 1, 1, 1);
            AssertRarity(actualT1, Rarity.Common, 1, 1, 1);

            AssertSetAndTotal(actualT2, "T2", 5, 2, 2);
            AssertRarity(actualT2, Rarity.Common, 5, 2, 2);

            AssertSetAndTotal(actualD1, "D1", 3, 2, 2);
            AssertRarity(actualD1, Rarity.Common, 3, 2, 2);

            AssertSetAndTotal(actualD2, "D2", 3, 1, 1);
